mod routing_table_test {
    ...
    use crate::servers::{networking::routing::{PdrEntry, PdrEstimator, RoutingTable}, NetworkGraph, INITIAL_PDR};
    ...
    fn test_new_with_graph() {
        ...
        assert_eq!(table.pdr_table, HashMap::from_iter([
    ...
    fn test_add_edge() {
        ...
        assert!(*table.graph.edge_weight(1, 2).unwrap() == 0.5);
        assert!(*table.graph.edge_weight(2, 1).unwrap() == 0.5);
    ...
    fn test_update_pdr_table() {
        let estimator: PdrEstimator = PdrEstimator::new(10, |_, acks, nacks| acks as f64 / (acks + nacks) as f64);
        let mut table: RoutingTable = RoutingTable::new_with_graph(NetworkGraph::new(), estimator);
        table.add_edge(1, 2);
    ...
    fn test_update_pdr_graph() {
        ...
        table.add_edge(2, 1);
        table.add_edge(1, 3);
        assert!(*table.graph.edge_weight(1, 2).unwrap() == 1. / INITIAL_PDR);
        assert!(*table.graph.edge_weight(1, 3).unwrap() == 1. / INITIAL_PDR);
        assert!(*table.graph.edge_weight(2, 1).unwrap() == 1. / INITIAL_PDR);
        for i in 0..10 {
            table.update_pdr(1, i < 2);
        }
        assert!(*table.graph.edge_weight(1, 2).unwrap() - 1. / 0.2 < 1e-6);
        assert!(*table.graph.edge_weight(1, 3).unwrap() - 1. / 0.2 < 1e-6);
        assert!(*table.graph.edge_weight(2, 1).unwrap() == 1. / INITIAL_PDR);
        for i in 0..10 {
            table.update_pdr(1, i < 8);
        }
        assert!(*table.graph.edge_weight(1, 2).unwrap() - 1. / 0.8 < 1e-6);
        assert!(*table.graph.edge_weight(1, 3).unwrap() - 1. / 0.8 < 1e-6);
        assert!(*table.graph.edge_weight(2, 1).unwrap() == 1. / INITIAL_PDR);
    }
